SESSION_NAME="cv_tiny"
echo "🚀 Starting 4 parallel CV experiments in tmux..."

# 기존 세션 종료
tmux kill-session -t $SESSION_NAME 2>/dev/null

tmux new-session -d -s $SESSION_NAME "bash scripts/run_tiny_res.sh"
sleep 0.2
tmux split-window -v -t $SESSION_NAME "bash scripts/run_tiny_res_pre.sh"
sleep 0.2
tmux split-window -h -t $SESSION_NAME:0.0 "bash scripts/run_tiny_vit.sh"
sleep 0.2
tmux split-window -h -t $SESSION_NAME:0.1 "bash scripts/run_tiny_vit_pre.sh"
sleep 0.2
